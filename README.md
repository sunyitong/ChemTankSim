...
| `liquid_level_sim/webapp/level_diff_bench.html` | Single-file web app: ROI box, difference heat map / overlay, adaptive robust diff, v3 multi-level detection (same code as `level_detect.py`), 33 bundled cases with ground truth, video mode (real-time frame-by-frame detection, multi-hypothesis temporal tracker, level-vs-time trace) |
...
6. Environment light: `scripts\env_map.py --id childrens_hospital`; fixtures, K group and video: `scripts\inserts_run.py --env childrens_hospital`, `scripts\lab_run.py`, `scripts\pour_video.py --env childrens_hospital` (→ `outputs/pour/F1_cyl_pour.mp4`), `scripts\eval_video.py`
