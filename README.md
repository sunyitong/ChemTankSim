...
| `liquid_level_sim/webapp/level_diff_bench.html` | Single-file web app: ROI box, difference heat map / overlay, adaptive robust diff, v3 multi-level detection (same code as `level_detect.py`), 35 bundled cases with ground truth, video mode for three pouring sequences (real-time frame-by-frame detection, multi-hypothesis temporal tracker, level-vs-time trace) |
