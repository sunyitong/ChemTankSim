...
│                                 差异热图 / 叠加模式，右侧 0–100% 刻度与液位估计；Adaptive 鲁棒差分；Layers 多液位；
│                                 顶栏按钮内嵌 10 组测试案例（含真值与误差显示）；纯前端，双击直接打开
│                                 重建：testset_eval.py → webapp_bundle.py → webapp_build.py <模板> <cases_embed.js> <输出>
